...
weight: cantiodad de interacciones entre ambos usuarios (en ese orden)
        ...
        for row in reader:
            ...
            if (row_data['commenter'] in dictionary):
                ...
                if index_responds_to >= 0:
                    ...
                    prev_interactions = dictionary[row_data['commenter']][index_responds_to][1]['interactions']
                    dictionary[row_data['commenter']][index_responds_to][1]['weight'] += 1
                    ...
                    dictionary[row_data['commenter']].append([row_data['responds_to'], { 'weight': 1, 'interactions': [new_comment] }])
            else:
                dictionary[row_data['commenter']] = [[row_data['responds_to'], { 'weight': 1, 'interactions': [new_comment] }]]
...
def save_graphml(csv_file):
    ...
    for user, respArray in respuestas_list:
        for response_to, response_dict in respArray:
            ...
            edge = (user, response_to, response_dict['weight'], response_dict['positive'], response_dict['interactions'])
            edges.append(edge)
    g = ig.Graph.TupleList(edges, edge_attrs=['weight', 'positive' ,'interactions'], directed=True)
